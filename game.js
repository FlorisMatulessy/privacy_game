...
function loadScenario(index) {
    ...
    document.getElementById('option-a').textContent = scenario.options.A;
    document.getElementById('option-b').textContent = scenario.options.B;
    document.getElementById('option-c').textContent = scenario.options.C;
    document.getElementById('option-d').textContent = scenario.options.D;
}
...
function showCorrectFeedback() {
    ...
    document.getElementById('chosen-answer').textContent = scenarios[currentScenario].options[scenarios[currentScenario].correct];
    ...
    const explanationSections = document.querySelectorAll('#feedback-screen .border-l-4');
    explanationSections.forEach(section => section.style.display = 'none');
...
function showWrongFeedback(answer) {
    ...
    document.getElementById('chosen-answer').textContent = `"${optionText}"`;
    document.getElementById('story-result').textContent = feedback.story;
    document.getElementById('explanation').textContent = feedback.explanation;
    document.getElementById('correct-answer-text').textContent = feedback.correctAnswer;
    
    // Show all sections for wrong answer
    const explanationSections = document.querySelectorAll('#feedback-screen .border-l-4');
    explanationSections.forEach(section => section.style.display = 'block');
...
function showScore() {
    ...
    // Calculate rank (mock)
    let rank = 10;
    if (score >= 90) rank = Math.floor(Math.random() * 3) + 1;
    else if (score >= 70) rank = Math.floor(Math.random() * 5) + 4;
    else rank = Math.floor(Math.random() * 10) + 8;
...
function showLeaderboard() {
    ...
    const rankNum = parseInt(document.getElementById('rank').textContent.substring(1));
    document.querySelector('#user-rank-display .w-10').textContent = rankNum;
    document.querySelector('#user-rank-display .text-green-500').textContent = userScore;
